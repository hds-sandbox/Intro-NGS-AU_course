...
The material for this course is organized in four separated jupyter notebooks in both `bash` and `python` where you will benefit of an interactive coding setup on `jupyterlab`.

The exercise lessons will work on a computing environment with `jupyterlab`. Use the menu `Access` and the drop-down menu selecting the computing environment you need (danish clusters `uCloud` and `GenomeDK`, your PC, or another cluster).

## Compiled exercises

if you need to have a look at the exercises as a reference, then the menu `Tutorials` contains all the compiled exercises on `jupyterlab` in a document format, from which you can also copy-paste the code.
...
- **Time:** 20 hours (for reading through the code, executing it, answering questions). The material fits 4-5 days of intensive whole-day lessons.
