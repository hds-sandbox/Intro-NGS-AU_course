...
title: "NGS data quality and alignment workflow with Galaxy"
author: "Samuele Soraggi, Lavinia Fechete"
...
There are 24 of these files, 12 for each of the two genotypes mentioned before. The files are named `[genotype]_[treatment]_[replicate].fastq`. Treatment 1 is before and
...
**1** Run Falco on the PacBio Hifi reads and on two of the Illumina RNA-seq libraries. Falco does quality control of the raw sequence data, providing an overview of the data which can help identify if there are any problems that should be addressed before further analysis. 
   
In the tool searchbar, find `Falco` and choose `Falco - An alternative, more performant [...]`. You will see a window with *tool parameters*: for the first option (raw read data from your current history), choose multiple files and select `Hifi_reads_white_clover.fastq` plus other `fastq` files you want to see the quality of (example in figure below).  Then click on the button `Run Tool`.
![](./images/falco.png)

You will notice that some new elements are added to your history. Part of them are `Falco` producing a text file, while others are `Falco` producing a webpage report. The reports are ready when coloured in green: click on the *eye symbol* of a history item to read a report.

**2** Falco provides a report for each sample. To have a better comparison between
the *Hifi* and *Illumina* data, we would combine the `Falco` reports into one using `MultiQC`.

Find the tool called `MultiQC aggregate results from [...]`. In the options, select `FastQC` as the used tool for the generated logs. Then select the history items of `FastQC` (equivalent to `Falco`) of your history producing `RawData` (Figure below). In this way, you build a pipeline from the previous reports to the new tool you are using. Now click on `Run Tool`.

![](./images/multiqc.png)

The tool will be now running in your history. When it is done, click on the *eye symbol* to see the report. It is easier to download it for better viewing.

&nbsp;

You can find a “Help” button that offers additional information about the plots for each panel. Otherwise, a more detailed guide of all information coming from the reports [is in this documentation](https://mugenomicscore.missouri.edu/PDF/FastQC_Manual.pdf) from page 5. 
...
**4** Run the same alignment, but choose as preset options the one for Short Reads ('sr').

&nbsp;

In the history, rename then the two alignments using the edit function (*pen symbol* in the history). Use for example names `Contig1_2_maphifi` and `Contig1_2_sr`, to distinguish alignment options and reference genome.
...
:::{.callout-tip title="Questions"}
Look at the alignments in IGV. 

- What do you notice about them? What is characteristic about the alignment `sr`? 

- Can you think of why `sr` alignment has these characteristics?

- What is the difference between the two alignments? 

&nbsp;

Especially for `sr` you are going to see a lot of graphics on the genome. Eventually, look online to find out what those mean. For example [those slides](https://www.igv.org/workshops/March2017/SlidesHandouts/IGV_SlideDeck.pdf) have a lot of visualizations from slide ~70.
:::

**8** Repeat the alignment with Minimap2 (using the chosen alignment option from the question above) and the sorting, but using the reference genomes for Contig 1 and for Contig 2 separaately. **Note:** you can run all at once by choosing multiple reference genomes in the options!
...
You can do this by selecting the libraries for each genotype and choosing `Advanced build list` --> `List of paired datasets`. 

![](./images/buildlist1.png)

Then click `Next` and define the pairs by `R1` and `R2` in their names, removing the default `_1` and `_2` suggested by Galaxy.

![](./images/buildList2.png)

Click `Next`, assign a name (TI or S10). You will have completed the process. Your sequences will be collected into two elements in your history, which makes usability better.

![](./images/builtlist.png)

**10** Do alignment of the RNA-seq lists of raw files to the reference `DNA_Contig1_2.fasta` using `RNA STAR Gapped-read mapper for RNA-seq data`. In the options use:
...
- as reference, `DNA_Contig1_2.fasta`, with Length of SA pre-indexing string equal to `9`. Usually you do not choose this value, and get a warning message to choose the right one if necessary.
...
:::{.calout-tip title="Questions"}
View the report to see the alignment statistics. What is the difference between *Aligned* and *Uniquely aligned* reads? Which factors could be responsible of having ~15% of reads not uniquely aligned? 
:::
